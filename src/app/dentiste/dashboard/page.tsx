      ...
      <div className="space-y-5 md:space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-3 md:gap-4">
          ...
          <div className="flex gap-2 md:gap-3 flex-wrap">
            <Link
              href="/patients"
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-3 md:px-4 py-2 bg-white text-[#191b23] border border-[#c3c6d6] rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#f3f3fd] transition-colors shadow-sm"
            >
              <UserPlus className="w-4 h-4" />
              <span className="hidden sm:inline">Nouveau </span>Patient
            </Link>
            <Link
              href="/consultations/nouvelle"
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-3 md:px-4 py-2 bg-[#0045a9] text-white rounded-lg text-xs sm:text-sm font-semibold hover:bg-[#003d96] transition-colors shadow-sm"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 md:gap-4">
          <div className="bg-white rounded-xl p-3 sm:p-4 md:p-6 shadow-sm border border-[#c3c6d6]/50 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <Calendar className="w-16 h-16 text-[#0045a9]" />
            </div>
            <p className="text-xs sm:text-sm text-[#424654] mb-1">RDV Aujourd&apos;hui</p>
            <h3 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#191b23]">
              {todayAppointments.length}
            </h3>
          </div>
          <div className="bg-white rounded-xl p-3 sm:p-4 md:p-6 shadow-sm border-l-4 border-l-[#006a6a] border border-[#c3c6d6]/50">
            <p className="text-xs sm:text-sm text-[#424654] mb-1">Confirmés</p>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#191b23]">{confirmed}</h3>
            <div className="mt-2 flex items-center gap-1 text-[#006a6a] text-xs font-semibold">
              <CheckCircle className="w-3 h-3" />
              Prêts
            </div>
          </div>
          <div className="bg-white rounded-xl p-3 sm:p-4 md:p-6 shadow-sm border-l-4 border-l-[#ac4100] border border-[#c3c6d6]/50">
            <p className="text-xs sm:text-sm text-[#424654] mb-1">Suivis</p>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#191b23]">{followUps}</h3>
            <div className="mt-2 flex items-center gap-1 text-[#853000] text-xs font-semibold">
              <Clock className="w-3 h-3" />
              Post-op
            </div>
          </div>
          <div className="bg-white rounded-xl p-3 sm:p-4 md:p-6 shadow-sm border-l-4 border-l-[#ba1a1a] border border-[#c3c6d6]/50">
            <p className="text-xs sm:text-sm text-[#424654] mb-1">Rappels</p>
            <h3 className="text-2xl sm:text-3xl font-bold text-[#191b23]">{reminders}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            {nextPatient && (
              <div className="bg-[#0045a9] text-white rounded-2xl p-4 sm:p-6 shadow-md relative overflow-hidden">
                <div className="absolute -right-10 -top-10 w-40 h-40 bg-white/10 rounded-full blur-2xl" />
                <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-5 relative z-10">
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-white/20 flex items-center justify-center text-white text-lg sm:text-xl font-bold border-2 border-white/30 flex-shrink-0">
                    {nextPatient.initials}
                  </div>
                  <div className="min-w-0">
                    <h4 className="text-base sm:text-xl font-bold truncate">{nextPatient.fullName}</h4>
                    <p className="text-xs sm:text-sm text-[#b1c5ff] truncate">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-[#c3c6d6]/50 overflow-hidden">
              <ul className="divide-y divide-[#c3c6d6]/30">
                {todayAppointments.map((apt) => (
                  <li
                    key={apt.id}
                    className={`flex items-center justify-between gap-2 p-3 sm:p-4 hover:bg-[#f3f3fd] transition-colors ${
                  ...
                  >
                    <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                      <div className="text-center w-10 sm:w-14 flex-shrink-0">
                        <div
                          className={`text-xs sm:text-sm font-bold ${apt.status === "patient-arrive" ? "text-[#0045a9]" : "text-[#191b23]"}`}
                        >
                          {apt.time}
                        </div>
                        <div className="text-[10px] sm:text-xs text-[#737785]">
                          {apt.duration} min
                        </div>
                      </div>
                      <div
                        className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${apt.colorClass}`}
                      >
                        {apt.patientInitials}
                      </div>
                      <div className="min-w-0">
                        <div className="text-sm font-semibold text-[#191b23] truncate">
                          {apt.patientName}
                        </div>
                        <div className="text-xs text-[#424654] truncate">
                          {apt.reason}
                        </div>
                      </div>
                    </div>
